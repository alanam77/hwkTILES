...
bool has_tiling(string floor)
{
	...
	unordered_map <int, Vertex*> Aset;
	unordered_map <int, Vertex*> Bset;
	Vertex* s = new Vertex();
	Vset.insert(s);
	Vertex* t = new Vertex();
	Vset.insert(t);
	...
	int numSpaces = 0;
	//Check if string has an Even amount of spaces
	...
	//Insert store Vertexs
	bool isRed = false;
	for (int i = 0; i < floor.size(); i++) {
		isRed = !isRed;
		if (width % 2 == 0 && floor[i] == '\n') {
			isRed = !isRed;
		}
		//If Current is Red
		if (isRed && floor[i] == ' ') {
			Aset[i] = new Vertex();
			s->neighs.insert(Aset[i]);
			s->weights[Aset[i]] = 1;
			Vset.insert(Aset[i]);
			if ((i - 1) > -1 && floor[i - 1] == ' ') {
				Aset[i]->neighs.insert(Bset[i - 1]);
				Aset[i]->weights[Bset[i - 1]] = 1;
			}
			if ((i - width) > -1 && floor[i - width] == ' ') {
				Aset[i]->neighs.insert(Bset[i - width]);
				Aset[i]->weights[Bset[i - width]] = 1;
			}
		}
		//Else if Current is not Red
		else if (!isRed && floor[i] == ' ') {
			Bset[i] = new Vertex();
			Bset[i]->neighs.insert(t);
			Bset[i]->weights[t] = 1;
			Vset.insert(Bset[i]);
			if ((i - 1) > -1 && floor[i - 1] == ' ') {
				Aset[i - 1]->neighs.insert(Bset[i]);
				Aset[i - 1]->weights[Bset[i]] = 1;
			}
			if ((i - width) > -1 && floor[i - width] == ' ') {
				Aset[i - width]->neighs.insert(Bset[i]);
				Aset[i - width]->weights[Bset[i]] = 1;
			}
		}
	}	
